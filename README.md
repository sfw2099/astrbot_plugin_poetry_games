...
Wordle 风格的猜诗句游戏！系统从**总库**随机选择一句 **4-7 字单句**作为答案，通过颜色反馈猜测。

```
/猜诗句               → 开始新游戏（随机长度 + 随机提示方式，先发空白框）
/猜诗句 4|5|6|7        → 指定答案字数
/猜诗句 声|形          → 指定提示方式（声=拼音 / 形=部首）
/猜诗句 4 声          → 组合指定（参数顺序任意）
cc 诗句               → 进行猜测（cc 后跟与答案字数一致的诗句）
/结束猜诗句            → 退出
/猜诗句帮助            → 查看规则
```
...
/诗词对垒 @某人   → 发起挑战
接受 / 拒绝        → 被挑战者响应
cc XXXX           → 私聊出题 / 群聊猜测（都带此前缀）
/结束对垒         → 取消对局
```

**规则**：
1. 出题字数随机 4/5/6/7 字，双方同字数
2. **私聊出题**：bot 分别私聊双方，各自私聊发「cc XXXX」（总库中），对方看不到题目
